        ...
        {visibleDrafts.map((draft, idx) => (
          ...
          >
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              {/* Traction warning — another contact at this company has positive signals */}
              {draft.company_traction?.has_traction && (
                <div className="flex items-start gap-2.5 rounded-md border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 px-3 py-2.5">
                  <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-400" />
                  <div className="space-y-0.5">
                    <p className="text-xs font-semibold text-amber-900 dark:text-amber-200">
                      Traction at this company — review before sending
                    </p>
                    <ul className="space-y-0.5">
                      {draft.company_traction.signals.map((sig: string, i: number) => (
                        <li key={i} className="text-xs text-amber-700 dark:text-amber-300">{sig}</li>
                      ))}
                    </ul>
                  </div>
                </div>
