...
const PHASE = '1'
const FILE  = '/Users/avanish/prospectIQ/.pipeline-queues/piq_p1_remaining.json'
const START = 0
const END   = 25
...
2. Decide the arm and get a real source URL. The record's "website" field is often EMPTY — do not let that cause a skip. Steps:
   a. If "website" is present, that is your Arm A fallback source URL.
   b. If "website" is empty/null, do ONE WebSearch for the company name to find its real official website/domain, and confirm it is a manufacturer (matches the equipment/pain_points on the record). Use that real URL as the source.
   c. Optionally (one quick search) look for a recent event -> ARM C (event-led, cite the real event URL). Else ARM A (operational-pain-led) using the website URL from (a)/(b).
   ONLY skip a company if, after the search, it has NO findable web presence OR is clearly NOT a manufacturer (e.g. a car dealership, a lender, a software/SaaS firm, a certification body). A genuine manufacturer with research on file must NOT be skipped just because the website field was blank — find the URL. Keep searches minimal (1-2 total).
