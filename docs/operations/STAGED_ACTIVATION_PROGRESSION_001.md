...
**Update (2026-08-15, session reconciliation) — the `daily_limit=1` reset above did not do what it was believed to do, and the SEND_ENABLED Flip Sequence and Emergency Freeze Procedure below had a real gap. Both are now fixed in code; recorded here so this document doesn't go stale on the same axis a second time.**

1. **`daily_limit` had no effect at dispatch time.** It was loaded in `engagement.py` but never passed into the send-path assertion battery — the actual ceiling was whatever `batch_size` × cron ticks produced (`batch_size=1` × 8 ticks/day ≈ 8/day), not the configured `daily_limit`. The 2026-08-14 reset above believed it was capping sends at 1/day; it was not enforcing any workspace-wide cap at all. **Fixed**: `daily_limit` is now enforced as a real workspace-wide cap in the assertion battery (`assert_workspace_under_daily_cap`), alongside the existing per-mailbox cap. `daily_limit=1` now means 1/day.
2. **`outreach_send_config.send_enabled` (the DB column) was read nowhere on the live dispatch path.** Only the Railway `SEND_ENABLED` env var was checked, and only by the scheduled `dispatch_loop` path — `POST /api/admin/trigger-dispatch` called the dispatch function directly, bypassing that check entirely. This meant: (a) the Emergency Freeze Procedure's step 4 (`UPDATE outreach_send_config SET send_enabled = false`) was a no-op — the procedure's step 2 (the Railway env var) was doing all the real work; (b) neither step protected against a call to `trigger-dispatch`, which could dispatch real email with `SEND_ENABLED=false` in Railway. **Fixed**: both the env var and the DB column are now checked together, fail-closed, at the single point every dispatch path goes through (`dispatch_workspace()` itself) — so `trigger-dispatch` is now gated too, and the DB column is no longer inert. The Flip Sequence and Emergency Freeze steps below are accurate as written; they just do more than they used to.
3. **A silent abort meant a Stage 0 observation could not actually observe anything.** `SEND_ENABLED=false` previously produced no log line and no result field — see the corresponding note in `DARK_LAUNCH_RUNTIME_OBSERVATION_004.md`. Fixed alongside the above; the abort is now a logged, queryable event.
4. **`outbound_queue` measured again, and it has not been idle.** 579 rows as of 2026-08-14 became **579 still** as of 2026-08-15 (all writers are dormant — the scheduler is halted), but broken down further: all 579 are sequence steps 2–5 (**zero step-1 drafts**), 554 were drafted in May 2026, 23 already carry `sent_at` (drain-only, duplicate-send risk if re-claimed), 3 queued contacts are now in `suppression_log`, and **321 of 579 (55%) cannot currently dispatch** because their prior sequence step was never sent — they would park 6h and re-fire on every future dispatch tick, the same pattern behind the 2026-06-05 retry storm described above. Recommendation: purge this queue (snapshot to a backup table first) and let a future activation generate a fresh schedule via `recompute_and_persist`, rather than inherit a 2-month-old one. Not urgent — the halt plus two independent CAN-SPAM blockers (see item 5) already prevent any send.
5. **A second CAN-SPAM blocker exists alongside `sender_physical_address`.** `BACKEND_PUBLIC_URL` is also unset in Railway production — see `DARK_LAUNCH_RUNTIME_OBSERVATION_004.md` §7 for detail. Both must be set before Stage 1.

None of the above un-voids this document or authorizes any stage transition. It corrects two mechanisms this document's own procedures depend on, so that when Stage 0 is eventually run for real, the tools it relies on behave as described.
