"""Pure Stripe operations — no DB dependencies.
...
This makes each operation independently testable by mocking the stripe module.
Adapters call these functions; they never call Stripe directly.
"""
...
from billing_core.types import InvoiceRecord, PaymentMethodInfo
from billing_core.tier_plans import TierPlan
...
def ensure_stripe_customer(
    ...
    The caller is responsible for persisting the returned ID back to the DB
    if it's newly created (use adapter.save_stripe_customer_id).
    """
    if customer_id:
        return customer_id
...
# ---------------------------------------------------------------------------
# Invoiced billing (annual / enterprise)
# ---------------------------------------------------------------------------
...
def create_and_send_invoice(
    ...
    months: int,
    net_days: int,
    product_name: str = "ProspectIQ",
) -> dict[str, Any]:
    """Create a Stripe Invoice item + invoice, finalise and send it.

    Returns a dict with id, invoice_url, pdf_url, amount_usd, due_date, status.
    """
    # Update customer email to AP billing address for this invoice
...
def list_invoices(stripe, customer_id: str, limit: int = 24) -> list[InvoiceRecord]:
    """Return up to `limit` invoices for the given Stripe customer."""
    try:
...
def get_default_payment_method(stripe, customer_id: str) -> PaymentMethodInfo | None:
    ...
    try:
        # Attempt 1: expand from customer object
...
def get_next_billing_date(stripe, subscription_id: str) -> int | None:
    """Return the Unix timestamp of the next billing date, or None on error."""
    try:
        sub = stripe.Subscription.retrieve(subscription_id)
        return sub.get("current_period_end")
    except Exception as exc:
        logger.warning("Failed to get subscription %s: %s", subscription_id, exc)
        return None


# ---------------------------------------------------------------------------
# Webhook event parsing (no DB write — return structured data for adapter)
# ---------------------------------------------------------------------------
