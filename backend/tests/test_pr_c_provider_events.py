...
def _resend_payload(
    event_type: str, email_id: str = MESSAGE_ID, include_created_at: bool = True
) -> dict:
    data: dict = {
        "email_id": email_id,
...
def _make_db(pe_raises=False) -> MagicMock:
    ...
    if pe_raises is False:
        ...
        pe_table.insert.return_value.execute.side_effect = Exception("connection pool exhausted")
...
def test_identical_delivered_is_deduped():
    ...
    with (
        _common_patches(db_new)[0],
        _common_patches(db_new)[1],
        _common_patches(db_new)[2],
        _common_patches(db_new)[3],
        _common_patches(db_new)[4],
        _common_patches(db_new)[5],
    ):
    ...
    with (
        _common_patches(db_dup)[0],
        _common_patches(db_dup)[1],
        _common_patches(db_dup)[2],
        _common_patches(db_dup)[3],
        _common_patches(db_dup)[4],
        _common_patches(db_dup)[5],
    ):
...
def test_identical_bounced_is_deduped():
    ...
    with (
        ...
        resp1 = TestClient(_make_app()).post(
            f"/api/webhooks/resend?secret={_TEST_WEBHOOK_SECRET}", json=payload
        )
    ...
    with (
        ...
        resp2 = TestClient(_make_app()).post(
            f"/api/webhooks/resend?secret={_TEST_WEBHOOK_SECRET}", json=payload
        )
