...
def test_heartbeat_today_start_uses_chicago_time():
    ...
    db_client = MagicMock()
    db_client.table.return_value.select.return_value.is_.return_value.execute.return_value = (
        MagicMock(data=[])
    )
    db_client.table.return_value.select.return_value.gte.return_value.limit.return_value.execute.return_value = MagicMock(
        count=5
    )
