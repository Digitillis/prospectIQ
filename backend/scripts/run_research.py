...
def main(
    company_ids: Optional[list[str]] = typer.Option(
        None, "--company-ids", help="Specific company IDs to research (repeatable)."
    ),
    batch_id: Optional[str] = typer.Option(
        None,
        "--batch-id",
        help="Research all companies tagged with this batch ID (from select_batch).",
    ),
    min_score: Optional[int] = typer.Option(
        None, "--min-score", help="Minimum firmographic PQS to select companies."
    ),
    limit: Optional[int] = typer.Option(
        None, "--limit", help="Max companies to research in this batch."
    ),
) -> None:
    ...
    result = agent.execute(
        company_ids=company_ids, batch_id=batch_id, min_firmographic_score=min_score, limit=limit
    )
