    ...
    try:
        q = db._filter_ws(
            db.client.table("contacts").select(
                "id,full_name,title,linkedin_url,linkedin_status,company_id,email,companies!contacts_company_id_fkey(id,name,domain,tier,pqs_total,status,industry,pain_signals,personalization_hooks)"
            )
        )
        if ls:
            q = q.in_("linkedin_status", ls)
        if at in ("connection", "dm"):
        ...
        for r in q.limit(count * 3).execute().data or []:
            ...
            if pqs < mi or pqs > ma:
                continue
            if inds and not any(i.lower() in (co.get("industry") or "").lower() for i in inds):
                continue
            if tkw and not any(k.lower() in (r.get("title") or "").lower() for k in tkw):
                continue
            res.append(
                {
                    "contact_id": r["id"],
                    ...
                    "pqs_total": pqs,
                    "pain_signals": co.get("pain_signals"),
                    "hooks": co.get("personalization_hooks"),
                }
            )
    ...
    return [
        {
            ...
            "source": src,
            "request_id": rid,
            "priority": max(0, 100 - (p.get("pqs_total", 0) or 0)),
            "pqs_at_queue_time": p.get("pqs_total", 0),
            "status": "pending",
            "scheduled_date": sd,
            "context": {k: p[k] for k in ("pain_signals", "hooks") if p.get(k)},
        }
        for p in prospects
    ]
...
async def request_actions(body: ActionRequestBody):
    db = get_db()
    sd = body.scheduled_date or _today_str()
    req = db.insert_action_request(
        {
    ...
    if items:
        try:
            inserted = db.insert_action_queue_batch(items)
        except Exception as e:
            db.update_action_request(rid, {"status": "failed", "error_message": str(e)})
            raise HTTPException(500, f"Failed: {e}")
    n = len(inserted)
    db.update_action_request(
        rid,
        {
            "fulfilled_count": n,
            "from_existing": n,
            "status": "fulfilled" if n >= body.count else ("partial" if n > 0 else "failed"),
        },
    )
    return {
        "data": {
            ...
            "queue_preview": [
                {
                    ...
                    "contact": p.get("full_name", ""),
                    "pqs": p.get("pqs_total", 0),
                }
                for p in prospects[:5]
            ],
        },
        "message": f"Queued {n} actions",
    }
...
async def get_queue(
    ...
    db = get_db()
    items = db.get_action_queue(date or _today_str(), action_type, status, limit)
...
async def complete_action(item_id: str, body: CompleteActionBody):
    ...
    db.insert_interaction(
        {
            "type": "note",
            "body": f"Action: {body.action_type}",
            "source": "daily_cockpit",
            "metadata": {"action_type": body.action_type, "queue_item_id": item_id},
            **(dict(company_id=body.company_id) if body.company_id else {}),
            **(dict(contact_id=body.contact_id) if body.contact_id else {}),
        }
    )
    if body.action_type == "connection" and body.contact_id:
        db.update_contact(
            body.contact_id, {"linkedin_status": "connection_sent", "updated_at": now}
        )
    if body.action_type == "dm" and body.contact_id:
        db.update_contact(body.contact_id, {"linkedin_status": "dm_sent", "updated_at": now})
...
async def remove_action(item_id: str):
    db = get_db()
    db._filter_ws(db.client.table("action_queue").delete()).eq("id", item_id).execute()
...
async def get_targets(date: str | None = None):
    db = get_db()
    tgts = db.get_daily_targets(date or _today_str())
...
async def update_target(body: UpdateTargetBody):
    db = get_db()
    r = db.upsert_daily_target(body.action_type, body.target_count, body.effective_date)
...
async def update_targets_batch(body: BatchTargetsBody):
    db = get_db()
    rs = [
        db.upsert_daily_target(i.action_type, i.target_count, body.effective_date)
...
async def auto_fill(scheduled_date: str | None = None):
    db = get_db()
    sd = scheduled_date or _today_str()
    tgts = db.get_daily_targets(sd)
    excl = db.get_queued_contact_ids(sd)
    total, by_type = 0, {}
    for t in tgts:
        ...
        if ps:
            ...
            try:
                db.insert_action_queue_batch(items)
                total += len(items)
                by_type[at] = len(items)
                excl.update(p["contact_id"] for p in ps if p.get("contact_id"))
            except:
                by_type[at] = 0
        else:
            by_type[at] = 0
    return {
        "data": {"scheduled_date": sd, "total_added": total, "by_type": by_type},
        "message": f"Auto-filled {total}",
    }
...
async def get_summary(date: str | None = None):
    db = get_db()
    sd = date or _today_str()
    tgts = db.get_daily_targets(sd)
    tm = {t["action_type"]: t["target_count"] for t in tgts}
    bd, td, tt = {}, 0, 0
    for at in list(tm.keys()) or ["connection", "dm", "email", "outcome", "post"]:
        d = db.count_action_queue(sd, at, "completed")
        p = db.count_action_queue(sd, at, "pending")
        t = tm.get(at, 0)
        bd[at] = {"done": d, "pending": p, "target": t}
        td += d
        tt += t
...
async def get_requests(limit: int = 50):
    db = get_db()
    r = db.get_action_requests(limit)
